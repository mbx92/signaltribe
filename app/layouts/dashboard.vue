<template>
  <div class="min-h-screen bg-base-200 flex flex-col">
    <!-- Top Header -->
    <header
      class="h-14 bg-base-100 border-b border-base-300 flex items-center px-4 lg:px-6 sticky top-0 z-40 shrink-0"
    >
      <!-- Logo -->
      <NuxtLink to="/" class="flex items-center gap-2 mr-6">
        <div
          class="w-7 h-7 bg-primary rounded flex items-center justify-center"
        >
          <IconChartArrows class="w-4 h-4 text-primary-content" />
        </div>
        <span class="font-bold text-primary text-base hidden lg:block">{{
          appName
        }}</span>
      </NuxtLink>
      ...
      <div class="flex items-center gap-2">
        <button class="btn btn-ghost btn-sm btn-square relative overflow-visible">
          <IconBell class="w-5 h-5" />
          <span
            class="absolute -top-0.5 -right-0.5 block h-2.5 w-2.5 rounded-full bg-primary ring-2 ring-base-100"
          ></span>
        </button>
        ...
        <div class="dropdown dropdown-end">
          <label
            tabindex="0"
            class="btn btn-ghost btn-sm flex items-center gap-2 px-2 cursor-pointer"
          >
            <div class="avatar">
              <div class="w-7 h-7 rounded-full">
                <img
                  :src="
                    currentUser?.avatar ?? 'https://i.pravatar.cc/150?u=guest'
                  "
                />
              </div>
            </div>
          ...
          <ul
            tabindex="0"
            class="dropdown-content menu menu-sm z-50 mt-1 w-52 bg-base-100 shadow-xl border border-base-200 rounded-box p-1"
          >
            <li class="menu-title px-3 pt-2 pb-1">
              <span class="text-xs"
                >Signed in as
                <strong>{{ currentUser?.name ?? "Guest" }}</strong></span
              >
            </li>
            <li class="px-3 pb-1">
              <span
                ...
                >{{ currentUser?.badge ?? "Guest" }}</span
              >
            </li>
            <li>
              <a class="gap-2"><IconUser class="w-4 h-4" /> My Profile</a>
            </li>
            <li>
              <a class="gap-2"><IconSettings class="w-4 h-4" /> Settings</a>
            </li>
            <div class="divider my-0.5"></div>
            <li>
              <a class="gap-2 text-error" @click="handleLogout"
                ><IconLogout class="w-4 h-4" /> Log Out</a
              >
            </li>
          </ul>
        </div>
    ...
    <div class="flex grow overflow-hidden">
      <!-- Sidebar -->
      <aside
        class="w-56 bg-base-100 border-r border-base-300 shrink-0 hidden lg:flex flex-col overflow-y-auto"
      >
        <nav class="flex flex-col gap-0.5 p-3 grow">
          <!-- User Section (visible to: user, analyst) -->
          <template v-if="showUserNav">
            <p
              class="text-xs font-semibold text-base-content/40 uppercase tracking-widest px-3 pt-3 pb-1"
            >
              My Account
            </p>
            <NuxtLink
              to="/dashboard/user"
              class="flex items-center gap-2.5 px-3 py-2 rounded text-sm font-medium text-base-content/70 hover:bg-base-200 hover:text-base-content transition-colors"
              active-class="bg-primary/10 text-primary font-semibold"
            >
              <IconLayoutDashboard class="w-4 h-4 shrink-0" /> Dashboard
            </NuxtLink>
            <NuxtLink
              to="/dashboard/user/pro-feed"
              class="flex items-center gap-2.5 px-3 py-2 rounded text-sm font-medium text-base-content/70 hover:bg-base-200 hover:text-base-content transition-colors"
              active-class="bg-primary/10 text-primary font-semibold"
            >
          ...
          <template v-if="showAnalystNav">
            <p
              class="text-xs font-semibold text-base-content/40 uppercase tracking-widest px-3 pb-1"
            >
              Analyst
            </p>
            <NuxtLink
            ...
            <NuxtLink
              to="/dashboard/analyst/subscribers"
              class="flex items-center gap-2.5 px-3 py-2 rounded text-sm font-medium text-base-content/70 hover:bg-base-200 hover:text-base-content transition-colors"
              active-class="bg-primary/10 text-primary font-semibold"
            >
          ...
          <template v-if="showAdminNav">
            ...
            <NuxtLink
              ...
              active-class="bg-primary/10 text-primary font-semibold"
            >
              <IconActivity class="w-4 h-4 shrink-0" /> System Logs
            </NuxtLink>
            <NuxtLink
              to="/dashboard/admin/api-docs"
              class="flex items-center gap-2.5 px-3 py-2 rounded text-sm font-medium text-base-content/70 hover:bg-base-200 hover:text-base-content transition-colors"
              active-class="bg-primary/10 text-primary font-semibold"
            >
              <IconBraces class="w-4 h-4 shrink-0" /> API Docs
            </NuxtLink>
            <NuxtLink
              to="/dashboard/admin/development"
              class="flex items-center gap-2.5 px-3 py-2 rounded text-sm font-medium text-base-content/70 hover:bg-base-200 hover:text-base-content transition-colors"
              active-class="bg-primary/10 text-primary font-semibold"
            >
              <IconTarget class="w-4 h-4 shrink-0" /> Development
            </NuxtLink>
            <NuxtLink
              ...
              active-class="bg-primary/10 text-primary font-semibold"
            >
              <IconFileDescription class="w-4 h-4 shrink-0" /> Legal Documents
            </NuxtLink>
          </template>
        </nav>
        ...
        <div class="p-3 border-t border-base-300">
          <NuxtLink
            to="/"
            class="flex items-center gap-2 px-3 py-2 rounded text-sm text-base-content/50 hover:text-base-content hover:bg-base-200 transition-colors"
          >
    ...
    <nav
      class="fixed bottom-0 left-0 right-0 z-99 bg-base-100 flex justify-around items-center h-16 lg:hidden border-t border-base-300 pb-0"
    >
      <!-- User View -->
      <template v-if="showUserNav && !showAnalystNav && !showAdminNav">
        <NuxtLink
          to="/dashboard/user"
          class="flex flex-col items-center justify-center p-2 w-full h-full text-base-content/60"
...
const { data: globalSettings } = await useAsyncData("settings", () =>
  $fetch("/api/settings"),
);
